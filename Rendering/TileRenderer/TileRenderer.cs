...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        // use data from Enum instead of string
        if (feature.Properties.Any(p => p.Key == LandTypes.highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
        ...
        // use data from Enum instead of string
        else if (feature.Properties.Any(p => p.Key == LandTypes.water) && feature.Type != GeometryType.Point)
        {
            var coordinates = feature.Coordinates;
        ...
        // use data from Enum instead of string
        else if (feature.Properties.Any(p => p.Key == LandTypes.railway))
        {
            var coordinates = feature.Coordinates;
        ...
        // use data from Enum instead of string
        else if (feature.Properties.Any(p => p.Key == LandTypes.natural && featureType == GeometryType.Polygon))
        {
            var coordinates = feature.Coordinates;
        ...
        // use data from Enum instead of string
        else if (feature.Properties.Any(p => p.Key == LandTypes.boundary && p.Value.StartsWith("forest")))
        {
            var coordinates = feature.Coordinates;
        ...
        // use data from Enum instead of string
        else if (feature.Properties.Any(p => p.Key == LandTypes.landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
        ...
        // use data from Enum instead of string
            p => p.Key == LandTypes.landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
        ...
        // use data from Enum instead of string
            p => p.Key == LandTypes.landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
        ...
        // use data from Enum instead of string
                 ...
                 feature.Properties.Any(p => p.Key == LandTypes.landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
        {
            var coordinates = feature.Coordinates;
        ...
        // use data from Enum instead of string
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == LandTypes.building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        // use data from Enum instead of string
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == LandTypes.leisure))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        // use data from Enum instead of string
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == LandTypes.amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
