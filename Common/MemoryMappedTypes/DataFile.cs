    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<LandTypes, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        // Add key to properties if it exists 
                        if(Enum.IsDefined(typeof(LandTypes), key.ToString()))
                        {
                            LandTypes keys_to_add = (LandTypes)Enum.Parse(typeof(LandTypes), key.ToString());
